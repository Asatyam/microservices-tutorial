...
import static com.satyamagrawal.bookstore.orderservice.testdata.TestDataFactory.*;
...
public class OrderControllerUnitTests {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("satyam");
    }

    @ParameterizedTest(name="[{index}] {0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreateOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("satyam"), any(CreateOrderRequest.class)))
                .willReturn(null);
